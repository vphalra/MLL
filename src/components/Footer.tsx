...
const Footer = () => {
  ...
  const scrollToIndustry = (industryIndex: number) => {
    if (location.pathname !== '/') {
      // If not on home page, navigate to home with state indicating which industry to show
      navigate('/', { state: { scrollToIndustry: true, industryIndex: industryIndex } });
    } else {
      // If already on home page, just scroll and select
      const industriesSection = document.getElementById('industries-section');
      if (industriesSection) {
        // First scroll to the industries section
        industriesSection.scrollIntoView({ behavior: 'smooth' });
        
        // Then set the slide to the correct industry
        setTimeout(() => {
          // Find the industries component and update its slide index
          const buttons = industriesSection.querySelectorAll('.flex.justify-center.mt-8.space-x-2 button');
          if (buttons && buttons[industryIndex]) {
            (buttons[industryIndex] as HTMLButtonElement).click();
          }
        }, 500);
      }
    }
